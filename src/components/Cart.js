...
export default function Cart({ cart, cartTotal, onClose, onUpdateQty, onClear, onOrderPlaced, whatsappNumber, upiId, shopName, contactPhone }) {
  ...
  const handlePlaceOrder = async (method) => {
    ...
    try {
      // Save to Firebase and get the doc ID for live status tracking
      const firebaseId = await placeOrder(orderData);
      const finalOrder = { ...orderData, firebaseId, placedAt: new Date().toISOString() };
      ...
      // Save to myOrders array in localStorage (all orders history)
      const existing = JSON.parse(localStorage.getItem("myOrders") || "[]");
      existing.unshift(finalOrder); // newest first
        ...
        <div className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-md flex flex-col bg-white"
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="text-center py-10 px-6"
              ...
              <p className="text-orange-100 mt-1 text-sm">Your order has been sent to us</p>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="rounded-2xl p-4 border-2 border-orange-200" style={{ background: "#FFF8F0" }}>
                ...
                <p className="text-sm text-gray-500 mb-3">
                  Contact us with your Order ID <strong>{confirmedOrder.orderId}</strong>
                </p>
                <a
                  href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Hi! Query about order ${confirmedOrder.orderId}. My name is ${confirmedOrder.customerName}.`)}`}
          ...
          <div className="p-4 border-t flex gap-3">
            <button
              onClick={onClose}
              className="flex-1 py-4 rounded-2xl text-white font-black text-base hover:brightness-110"
              style={{ background: "linear-gradient(135deg,#FF6B35,#FF8C42)" }}
            >
              Back to Menu 🍽️
            </button>
      ...
      <div className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-md flex flex-col bg-white"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b">
          ...
          <div className="flex gap-2">
            {cart.length > 0 && (
              <button onClick={onClear} className="text-xs text-red-400 hover:text-red-600 border border-red-200 px-3 py-1 rounded-full transition">
                Clear All
              </button>
            )}
            <button onClick={onClose} className="w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 font-bold">✕</button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          {cart.length === 0 ? (
            ...
            <>
              {cart.map(item => (
                <div key={item.id} className="flex items-center gap-3 bg-orange-50 rounded-2xl p-3">
                  ...
                  <div className="flex items-center gap-2 bg-white rounded-xl p-1">
                    <button onClick={() => onUpdateQty(item.id, -1)} className="w-7 h-7 rounded-lg bg-orange-100 text-orange-500 font-black flex items-center justify-center hover:bg-orange-200">−</button>
              ...
              {/* Customer details — remembers name/phone */}
          ...
          <div className="border-t px-5 py-5 space-y-3" style={{ background: "#FFF8F0" }}>
            ...
            {paymentMode === "whatsapp" && (
              <div className="bg-green-50 rounded-2xl p-4 text-center">
                <p className="text-sm text-green-700 mb-3">Order via WhatsApp — pay on delivery or pickup!</p>
                <button
            ...
            {paymentMode === "upi" && (
              <div className="bg-purple-50 rounded-2xl p-4 text-center">
                ...
                <p className="text-xs text-purple-500 mb-3">Scan QR or tap Pay Now</p>
